# Kurnia Surya - Data Analyst / Data Engineer

👋 Hello and welcome!

I'm **Kurnia Surya**, a Data Analyst and Data Engineer passionate about transforming raw data into impactful insights and build a strong data pipeline. With solid training from Hacktiv8's Comprehensive Data Analyst and diverse experience in government agency. I use a data-driven approach to tackle problems at work, and I'm eager to apply that in the business world to help organizations make decisions that are both accurate and impactful.


---

## 🚀 About Me

- 🌱 **Currently Learning:** Cloud-based data transformation using dbt and AWS.
- 👯 **Collaboration Interests:** Open to teaming up on end-to-end data solutions, from wrangling and cleaning raw data to building insightful dashboards, automated reporting pipelines, and data-driven recommendations for business decisions.
- 🌐 **Background Insight:** My experience in government agencies has given me a distinct perspective on how data plays a critical role in professional environments, driving my passion to develop practical and impactful solutions.
- 🎭 **Fun Fact:** I've built strong coordination and communication skills through my work in government institutions, engaging with diverse leaders and individuals across various levels — which has also sharpened my ability to deliver clear, concise reports to company executives.

---

## 📫 Connect with Me

- LinkedIn: [Connect on LinkedIn](https://www.linkedin.com/in/kurniasuryab/)
- Email: kurniasuryab@gmail.com

---

## 🛠 Skills

- **Languages:** Python, SQL
- **Data Analysis & Visualization:** Tableau, Google Looker Studio, Pandas, Matplotlib, Seaborn, Plotly
- **Database Management:** PostgreSQL, NeonDB, MongoDB
- **Data Engineering & Pipeline:** Apache Airflow, Apache Kafka, Apache Spark
- **Version Control & Collaboration:** Git, GitHub, Docker
- **Other Skills:** ETL Development, Data Wrangling, Real-time Data Pipelines, Data Modeling, Data Cleaning, Statistical Analysis

---

## 🎓 Education

### Hacktiv8 Comprehensive Data Analyst Bootcamp
Core Skills Developed:
Data Wrangling, Visualization, Statistical Analysis, SQL, Data Pipeline, Automation.

---

Thanks for visiting! Explore my work, drop a message, and let's build something great together in the world of data analytics and engineering!
